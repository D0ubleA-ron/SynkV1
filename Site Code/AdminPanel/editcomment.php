...
if (isset($_GET["commentId"])) {
    ...
    $postid = $conn->real_escape_string($commentId);
    $query = "SELECT commentContent FROM Comment WHERE commentId = ?";
...
?>
    ...
    <form method="post">
        <div style="text-align: center;margin-top: 10em">
            <p style="color: #313131">Comment</p>
            <input type="text" name="commentContent" id="usernamereset" style="display: inline-block;border-color: #313131; outline-width: 1em;" value="<?php echo $commentContent?>" required>
...
if(isset($_POST["commentContent"]))
{
    ...
    $newcomment = $_POST["commentContent"];

    $newcomment = $conn->real_escape_string($newcomment);
    $query = "UPDATE Comment SET commentContent = ? WHERE commentId = ?";
    $stmt2 = $conn->prepare($query);
    $stmt2->bind_param("si", $newcomment,$commentId);
    $stmt2->execute();
