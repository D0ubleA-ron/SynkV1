...
<?php if(isset($_POST['dropdown'])) { ?>
    ...
    $postid = $_GET['postId'];
    $postitle = $_GET['postTitle'];



    $stmt = $conn->prepare("SELECT DATE_FORMAT(date, ?) AS hour, COUNT(userid) AS activity FROM Tracking GROUP BY hour;");
...
<div style="margin-top: 2rem">
    <p style="color: #313131; text-align: center">Site Usage Report by <?php echo $daytype?></p>
    ...
    <script>
        ...
        var myChart = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [{
                    label: 'Activity per <?php echo $daytype; ?>',
                    data: values,
                    fill: false,
